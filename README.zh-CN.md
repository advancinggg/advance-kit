...
| 插件 | 版本 | 状态 |
|---|---|---|
| `dev` | `2.1.0` | 稳定版——AC 驱动的进度公式；/dev ↔ /spec 章节编号全面对齐（新增 §2.12 State Management、§3.8 Implementation Notes；§1.1 改名为 "Module Goals & Overview"）；包含 `dev` / `spec` skill 和可选的 statusline。**迁移说明**：已存在的 MODULE doc 在下次 `/spec` rerun 时**不会**自动获得 §2.12/§3.8 — 需手动补充或使用 "Regenerate all"。 |
