...
|---|---|---|
| `dev` | `3.2.0` | Estable — flujo /dev + /spec + /prd con evaluadores duales, progreso MODULE por AC, ADR system, /dev paralelo por worktree, panel snapshot de solo lectura `/dev board`, capa de aceptación de sistema, visibilidad de deriva de versión de plantilla. Incluye skills `dev` / `spec` / `prd` + statusline opcional. **Más reciente:** 3.2.0 /dev añade un contrato de harness de aceptación de sistema — detecta/declara cómo levantar + ejecutar el sistema real cableado para los tests e2e de SYS-AC (witness-floor: sin mocks), más un aplazamiento explícito aceptado por el usuario para que un REQ `e2e` nunca quede en silencio atascado en `Partial` cuando el sistema cableado no puede ejecutarse aquí. **Anterior:** 2.12.0 visibilidad de deriva de versión de plantilla (banner + deriva sesión↔instalada + sellos de artefacto), 3.0.0 descubrimiento de journeys respaldado por evaluadores + Phase 1.3 descubrimiento de journeys emergentes, 3.1.0 criterios SYS-AC atómicos adjudicables (§1.1). |
