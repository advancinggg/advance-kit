...
|---|---|---|
| `dev` | `3.0.0` | Estable — flujo /dev + /spec + /prd con evaluadores duales, progreso MODULE por AC, ADR system, /dev paralelo por worktree, panel snapshot de solo lectura `/dev board`, capa de aceptación de sistema, visibilidad de deriva de versión de plantilla. Incluye skills `dev` / `spec` / `prd` + statusline opcional. **Más reciente:** 3.0.0 `/spec upgrade-template` ejecuta descubrimiento de journeys respaldado por evaluadores (dual, loop-until-dry — REQs sub-clasificados + journeys emergentes entre módulos; degrada a single/heurístico) y Phase 1.3 se refuerza a descubrimiento loop-until-dry (MAJOR: rompe el previo contrato UT.10 no-evaluator). **Anterior:** 2.10.0 capa de aceptación de sistema, 2.11.0 `/spec upgrade-template` (adopta en proyectos existentes sin rerun completo), 2.12.0 visibilidad de deriva de versión de plantilla (banner + deriva sesión↔instalada + sellos de artefacto). |
