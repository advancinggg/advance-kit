...
|---|---|---|
| `dev` | `3.8.0` | Estable — flujo /dev + /spec + /prd con evaluadores duales, progreso MODULE por AC, ADR system, /dev paralelo por worktree, panel snapshot de solo lectura `/dev board`, capa de aceptación de sistema, visibilidad de deriva de versión de plantilla. Incluye skills `dev` / `spec` / `prd` + statusline opcional. **Más reciente:** 3.8.0 hace mecánica la paridad de ledger §1.5↔§3.4 de 3.7.0 — una nueva compuerta PreToolUse de salida de DOCS (fail-open: `check-phase.sh` + `ledger-parity-check.sh`) impide que un /dev salga de DOCS mientras algún módulo en alcance tenga un AC de §1.5 sin su fila §3.4, nombrando los AC-IDs huérfanos. **Anterior:** 3.7.0 paridad de ledger §1.5↔§3.4 (§1.5 autoritativo, /dev DOCS crea filas §3.4, SUMMARY/§6.1.1/evaluador fail-closed + compuerta DoD Ledger Parity), 3.6.0 `/dev board` muestra el eje de aceptación de sistema (deferrals + desglose atómico por tipo + columna DEFER por worktree) con persistencia durable en §3, 3.5.0 /prd marcador en cada flujo §3 + backfill heredado. |
