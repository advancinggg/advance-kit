...
|---|---|---|
| `dev` | `3.1.0` | Estable — flujo /dev + /spec + /prd con evaluadores duales, progreso MODULE por AC, ADR system, /dev paralelo por worktree, panel snapshot de solo lectura `/dev board`, capa de aceptación de sistema, visibilidad de deriva de versión de plantilla. Incluye skills `dev` / `spec` / `prd` + statusline opcional. **Más reciente:** 3.1.0 los journeys de aceptación de sistema se descomponen en criterios SYS-AC atómicos e independientemente adjudicables (≥1 funcional + NFR/SLO + ruta-de-error por journey en `docs/SYSTEM-ACCEPTANCE.md` §1.1; el ledger §2 solo-estado + IDs de 2 dígitos sin cambios). **Anterior:** 2.11.0 `/spec upgrade-template` (adopta en proyectos existentes sin rerun completo), 2.12.0 visibilidad de deriva de versión de plantilla (banner + deriva sesión↔instalada + sellos de artefacto), 3.0.0 descubrimiento de journeys respaldado por evaluadores + Phase 1.3 descubrimiento loop-until-dry de journeys emergentes. |
