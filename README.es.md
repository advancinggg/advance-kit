...
|---|---|---|
| `dev` | `3.4.0` | Estable — flujo /dev + /spec + /prd con evaluadores duales, progreso MODULE por AC, ADR system, /dev paralelo por worktree, panel snapshot de solo lectura `/dev board`, capa de aceptación de sistema, visibilidad de deriva de versión de plantilla. Incluye skills `dev` / `spec` / `prd` + statusline opcional. **Más reciente:** 3.4.0 la Iron Rule ahora distingue suavizar un hallazgo vivo del evaluador (prohibido) de declarar un límite de herramienta/alcance (requerido), y cada resumen de finalización (/dev SUMMARY, /spec Final Report + UT.9, /prd HANDOFF) gana un campo "Scope & unverified" estructurado y de origen mecánico. **Anterior:** 3.1.0 criterios SYS-AC atómicos adjudicables (§1.1), 3.2.0 harness de aceptación de sistema + ejecutabilidad (declara/ejecuta el e2e cableado + deferral explícito), 3.3.0 `/spec upgrade-template` renumber-preserve al insertar una sección a mitad de plantilla. |
