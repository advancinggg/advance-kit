...
| `telegram-channels-pro` | `0.1.1` | v0.1 completo (solo macOS) — 8 módulos: daemon-core + telegram-client + mcp-server-proxy + admin-auth + observability + mcp-tools (5 herramientas MCP) + routing (LRU + control de admin + comandos slash) + deployment (CLI launchd + socket de control + ROLLBACK.md). |
