...
|---|---|---|
| `dev` | `3.5.0` | Estable — flujo /dev + /spec + /prd con evaluadores duales, progreso MODULE por AC, ADR system, /dev paralelo por worktree, panel snapshot de solo lectura `/dev board`, capa de aceptación de sistema, visibilidad de deriva de versión de plantilla. Incluye skills `dev` / `spec` / `prd` + statusline opcional. **Más reciente:** 3.5.0 /prd ahora exige el marcador `System acceptance journey? Yes/No` en CADA flujo §3 (Phase 4 marca uno faltante), más un backfill heredado (por lotes, clasificado por el usuario) para que los PRD pre-2.10.0 adopten el marcador sin regenerar por completo. **Anterior:** 3.2.0 harness de aceptación de sistema + ejecutabilidad (declara/ejecuta el e2e cableado + deferral explícito), 3.3.0 `/spec upgrade-template` renumber-preserve al insertar una sección a mitad de plantilla, 3.4.0 carve-out de divulgación honesta de la Iron Rule + campo "Scope & unverified" estructurado. |
