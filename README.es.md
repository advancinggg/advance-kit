...
|---|---|---|
| `dev` | `3.7.0` | Estable — flujo /dev + /spec + /prd con evaluadores duales, progreso MODULE por AC, ADR system, /dev paralelo por worktree, panel snapshot de solo lectura `/dev board`, capa de aceptación de sistema, visibilidad de deriva de versión de plantilla. Incluye skills `dev` / `spec` / `prd` + statusline opcional. **Más reciente:** 3.7.0 paridad de ledger §1.5↔§3.4 — §1.5 es la fuente autoritativa de AC y /dev DOCS crea las filas §3.4 en el mismo commit, de modo que SUMMARY, la compuerta de progreso §6.1.1 y el evaluador de módulo de /spec fallan en cerrado ante un AC con fila pendiente (cerrando la clase de over-claim donde un AC declarado-pero-sin-fila dejaba que un REQ leyera Verified), respaldado por una nueva compuerta DoD de Ledger Parity. **Anterior:** 3.6.0 `/dev board` muestra el eje de aceptación de sistema (deferrals + desglose atómico por tipo + columna DEFER por worktree) con persistencia durable en §3, 3.5.0 /prd marcador en cada flujo §3 + backfill heredado, 3.4.0 carve-out de divulgación honesta de la Iron Rule + campo "Scope & unverified". |
