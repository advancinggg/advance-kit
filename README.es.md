...
|---|---|---|
| `dev` | `2.8.0` | Estable — fórmula de progreso basada en AC; numeración de secciones /dev ↔ /spec alineada. Incluye la skill **`/prd`** para generación iterativa de PRD (diálogo guiado one-question-per-turn + evaluador de cobertura de 4 dimensiones — adaptado del skill de brainstorming de Jesse Obra). La plantilla MODULE incorpora §1.1 "Serves PRD topics" (mapeo inverso), §2.13 Operations (runbook) y §2.14 Observability (esquema de logs/métricas/trazas). **2.4.0 agrega** enrutamiento `docs/CONTEXT-MAP.md` (auto-generado por /spec Phase 3.3) y resolución de términos `docs/GLOSSARY.md` (bootstrap por /prd Phase 3.3, /spec Phase 2.6 agrega conceptos técnicos); /dev PLAN acota el contexto al scope coincidente con verificación de staleness en python3 multiplataforma. **2.5.0 añade** flujo de ADR (Architecture Decision Records): `/spec adr-new` crea archivos de decisión con fecha bajo `docs/adr/` desde plantilla incrustada, mantiene automáticamente `_INDEX.md`, Phase 1 detecta conflictos por pares sobre ADR Aceptados (22 pares de palabras clave opuestas + regla de proximidad de marcador de decisión), e integra ADR compliance en /dev PLAN + DOCS. **2.6.0 refina la disciplina de alcance de /prd**: Phase 4 Dimension 4 detecta fugas de arquitectura (IDs de módulo, DDL, tablas de rutas de API, firmas de traits, diagramas entre módulos, layouts de directorio → Critical), añade sub-secciones opcionales de plantilla §1.1 Design principles (invariantes de producto transversales) y §7.1 Milestones / rollout (hoja de ruta por capacidad de usuario, ortogonal a /spec IMPLEMENTATION_ORDER.md), y un bloque de frontera en Phase 3.0 que separa el ámbito PRD del ámbito /spec. **2.7.0 agrega** puntos de alineación upstream: /dev DOCS §2.1.2 (comprobación de cambio PRD/Spec — abort+restart como §2.1.1 ADR), §2.1.3 (comprobación de deriva en Core Logic — auditoría §2.7 solo en re-entrada tras rollback) y /spec Phase 0.6 (escalación de brechas de PRD — formaliza la ruta upstream-rollback del Iron Rule de /spec como AskUserQuestion estructurado). **2.8.0 agrega** /dev paralelo por worktree: 4 subcomandos (`worktree-new` / `worktree-list` / `worktree-finish` / `worktree-remove`) respaldados por `plugins/dev/bin/worktree-helper.sh`. Cada worktree obtiene naturalmente su propio `.dev-state/` vía `git rev-parse --show-toplevel`. Las secuencias 2.7.0 §2.1.2 Option A (4 comandos) y Option B (3 comandos) permanecen UNCHANGED — el bridging en modo worktree (cd + git commit + git rebase usando ref LOCAL vía el `.git/` object store compartido) vive en la narrativa §8.2, no como comandos alternativos. /spec + /prd siguen single-flight; §8 documenta la convención main-worktree-only + la realidad de auto-push de stop.sh con 5 puertas + contención de `.git/index.lock` como peculiaridades operativas aceptadas. Incluye skills `dev` / `spec` / `prd` y statusline opcional. |
