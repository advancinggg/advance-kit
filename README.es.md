...
|---|---|---|
| `dev` | `2.12.0` | Estable — flujo /dev + /spec + /prd con evaluadores duales, progreso MODULE por AC, ADR system, /dev paralelo por worktree, panel snapshot de solo lectura `/dev board`, capa de aceptación de sistema, visibilidad de deriva de versión de plantilla. Incluye skills `dev` / `spec` / `prd` + statusline opcional. **Más reciente:** 2.12.0 cada skill imprime la versión de plantilla dev en ejecución y avisa de deriva sesión↔instalada (un plugin más nuevo instalado a mitad de sesión); /spec y /prd sellan los documentos generados con `dev-template: vX.Y.Z` y marcan deriva de artefacto al re-ejecutar. **Anterior:** 2.9.0 panel snapshot `/dev board`, 2.10.0 capa de aceptación de sistema, 2.11.0 `/spec upgrade-template` (adopta aceptación de sistema en proyectos existentes sin rerun completo). |
