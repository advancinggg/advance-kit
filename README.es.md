...
|---|---|---|
| `dev` | `2.11.0` | Estable — flujo /dev + /spec + /prd con evaluadores duales, progreso MODULE por AC, ADR system, /dev paralelo por worktree, panel snapshot de solo lectura `/dev board`, capa de aceptación de sistema. Incluye skills `dev` / `spec` / `prd` + statusline opcional. **Más reciente:** 2.11.0 `/spec upgrade-template` adopta la capa de aceptación de sistema en proyectos existentes sin rerun completo (inyecta la columna `Witness` del registro + bootstrapea `docs/SYSTEM-ACCEPTANCE.md`; sin evaluadores, idempotente). **Anterior:** 2.8.0 /dev paralelo por worktree, 2.9.0 panel snapshot `/dev board`, 2.10.0 capa de aceptación de sistema. |
