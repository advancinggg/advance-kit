...
| `telegram-channels-pro` | `0.1.0` | Corte de infraestructura v0.1 (solo macOS) — daemon-core + telegram-client + mcp-server-proxy (lado daemon) + admin-auth + observability. **Aún no enviado:** mcp-tools / routing / CLI de despliegue (próxima versión). |
