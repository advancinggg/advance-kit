...
|---|---|---|
| `dev` | `3.3.0` | Estable — flujo /dev + /spec + /prd con evaluadores duales, progreso MODULE por AC, ADR system, /dev paralelo por worktree, panel snapshot de solo lectura `/dev board`, capa de aceptación de sistema, visibilidad de deriva de versión de plantilla. Incluye skills `dev` / `spec` / `prd` + statusline opcional. **Más reciente:** 3.3.0 `/spec upgrade-template` hace renumber-preserve al insertar una sección a mitad de plantilla — UT.3.0 empareja secciones por título (no por número), así insertar una nueva sección numerada ya no retitula-corrompe los docs existentes (las secciones desplazadas conservan cuerpo+título, solo se corrige el número; subtítulos + refs § cascadeados; la ambigüedad pregunta). **Anterior:** 3.0.0 descubrimiento de journeys respaldado por evaluadores + Phase 1.3 descubrimiento de journeys emergentes, 3.1.0 criterios SYS-AC atómicos adjudicables (§1.1), 3.2.0 harness de aceptación de sistema + ejecutabilidad (declara/ejecuta el e2e cableado + deferral explícito). |
