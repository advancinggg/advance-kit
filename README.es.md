...
|---|---|---|
| `dev` | `3.6.0` | Estable — flujo /dev + /spec + /prd con evaluadores duales, progreso MODULE por AC, ADR system, /dev paralelo por worktree, panel snapshot de solo lectura `/dev board`, capa de aceptación de sistema, visibilidad de deriva de versión de plantilla. Incluye skills `dev` / `spec` / `prd` + statusline opcional. **Más reciente:** 3.6.0 `/dev board` muestra el eje de evaluación de aceptación de sistema — deferrals (con motivo), desglose atómico por tipo functional/nfr/error-path y una columna DEFER por worktree — respaldado por la persistencia durable de deferrals en §3 de SYSTEM-ACCEPTANCE.md. **Anterior:** 3.5.0 /prd exige el marcador de aceptación de sistema en cada flujo §3 + backfill heredado, 3.4.0 carve-out de divulgación honesta de la Iron Rule + campo "Scope & unverified", 3.3.0 `/spec upgrade-template` renumber-preserve al insertar una sección a mitad de plantilla. |
