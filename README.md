...
|---|---|---|
| `dev` | `3.4.0` | Stable — enforced /dev + /spec + /prd with dual-model evaluators, AC-driven MODULE progress, ADR system, worktree-parallel /dev, read-only `/dev board` snapshot dashboard, system-acceptance witness layer, template-version drift visibility. Includes `dev`, `spec`, `prd` skills + opt-in statusline. **Latest:** 3.4.0 the Iron Rule now distinguishes softening a live evaluator finding (forbidden) from declaring a tool/scope boundary (required), and every completion summary (/dev SUMMARY, /spec Final Report + UT.9, /prd HANDOFF) gains a structured, mechanically-sourced "Scope & unverified" field. **Earlier:** 3.1.0 atomic, independently-adjudicable SYS-AC criteria (§1.1), 3.2.0 system-acceptance harness + executability (declare/run the wired e2e + explicit deferral), 3.3.0 `/spec upgrade-template` renumber-preserve on a mid-template section insert. |
