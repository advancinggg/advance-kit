...
|---|---|---|
| `dev` | `3.5.1` | Stable — enforced /dev + /spec + /prd with dual-model evaluators, AC-driven MODULE progress, ADR system, worktree-parallel /dev, read-only `/dev board` snapshot dashboard, system-acceptance witness layer, template-version drift visibility. Includes `dev`, `spec`, `prd` skills + opt-in statusline. **Latest:** 3.5.0 /prd now requires the `System acceptance journey? Yes/No` marker on every §3 flow (Phase 4 flags a missing one), plus a batched, user-classified legacy backfill so pre-2.10.0 PRDs adopt the marker without a full regenerate. **Earlier:** 3.2.0 system-acceptance harness + executability (declare/run the wired e2e + explicit deferral), 3.3.0 `/spec upgrade-template` renumber-preserve on a mid-template section insert, 3.4.0 Iron-Rule honest-disclosure carve-out + structured "Scope & unverified" field. |
