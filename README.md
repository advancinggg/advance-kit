...
| `telegram-channels-pro` | `0.1.0` | v0.1 infrastructure slice (macOS only) — daemon-core + telegram-client + mcp-server-proxy (daemon side) + admin-auth + observability. **Not yet shipped:** mcp-tools / routing / deployment CLI (subsequent release). |
