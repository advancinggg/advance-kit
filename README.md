...
|---|---|---|
| `dev` | `3.3.0` | Stable — enforced /dev + /spec + /prd with dual-model evaluators, AC-driven MODULE progress, ADR system, worktree-parallel /dev, read-only `/dev board` snapshot dashboard, system-acceptance witness layer, template-version drift visibility. Includes `dev`, `spec`, `prd` skills + opt-in statusline. **Latest:** 3.3.0 `/spec upgrade-template` renumber-preserves on a mid-template section insert — UT.3.0 matches sections by title (not number), so inserting a new numbered section no longer retitle-corrupts existing docs (shifted sections keep body+title, only the number is fixed; subheadings + §-refs cascaded; ambiguity prompts). **Earlier:** 3.0.0 evaluator-backed journey discovery + Phase 1.3 emergent-journey discovery, 3.1.0 atomic, independently-adjudicable SYS-AC criteria (§1.1), 3.2.0 system-acceptance harness + executability (declare/run the wired e2e + explicit deferral). |
