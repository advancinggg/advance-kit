...
|---|---|---|
| `dev` | `2.5.0` | Stable — AC-based progress formula; /dev ↔ /spec section numbering aligned. Ships **`/prd` skill** for iterative PRD generation (guided dialogue, one-question-per-turn, 4-dimension coverage evaluator — adapted from Jesse Obra's brainstorming skill). MODULE template gains §1.1 "Serves PRD topics" reverse mapping, §2.13 Operations (runbook), §2.14 Observability (log/metric/trace schema). **2.4.0** adds `docs/CONTEXT-MAP.md` routing (auto-generated by /spec Phase 3.3) and `docs/GLOSSARY.md` domain-term resolution (bootstrapped by /prd Phase 3.3, appended to by /spec Phase 2.6); /dev PLAN narrows context to the matched scope entry with cross-platform python3 staleness check. **2.5.0** ships an ADR (Architecture Decision Records) workflow: `/spec adr-new` creates date-named decision files under `docs/adr/` from an inline template, auto-maintains `_INDEX.md`, Phase 1 detects semantic conflicts pairwise over Accepted ADRs (22 opposing-keyword pairs + decision-marker proximity), and wires ADR compliance into /dev PLAN + DOCS. Includes `dev`, `spec`, `prd` skills plus opt-in statusline. |
