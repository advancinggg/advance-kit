...
|---|---|---|
| `dev` | `3.8.0` | Stable — enforced /dev + /spec + /prd with dual-model evaluators, AC-driven MODULE progress, ADR system, worktree-parallel /dev, read-only `/dev board` snapshot dashboard, system-acceptance witness layer, template-version drift visibility. Includes `dev`, `spec`, `prd` skills + opt-in statusline. **Latest:** 3.8.0 mechanically enforces the 3.7.0 §1.5↔§3.4 ledger parity — a new fail-open DOCS-exit PreToolUse gate (`check-phase.sh` + `ledger-parity-check.sh`) blocks a /dev run from leaving DOCS while any in-scope module's §1.5 AC is missing its §3.4 row, naming the orphan AC-IDs. **Earlier:** 3.7.0 §1.5↔§3.4 ledger parity (§1.5 authoritative, /dev DOCS births §3.4 rows, fail-closed SUMMARY/§6.1.1/evaluator + Ledger Parity DoD gate), 3.6.0 `/dev board` surfaces the system-acceptance axis (deferrals + atomic Type breakdown + per-worktree DEFER column) with durable §3 persistence, 3.5.0 /prd marker on every §3 flow + legacy backfill. |
