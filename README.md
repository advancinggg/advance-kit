...
|---|---|---|
| `dev` | `3.1.0` | Stable — enforced /dev + /spec + /prd with dual-model evaluators, AC-driven MODULE progress, ADR system, worktree-parallel /dev, read-only `/dev board` snapshot dashboard, system-acceptance witness layer, template-version drift visibility. Includes `dev`, `spec`, `prd` skills + opt-in statusline. **Latest:** 3.1.0 system-acceptance journeys decompose into atomic, independently-adjudicable SYS-AC criteria (≥1 functional + NFR/SLO + error-path per journey in `docs/SYSTEM-ACCEPTANCE.md` §1.1; the status-only §2 ledger + 2-digit IDs unchanged). **Earlier:** 2.11.0 `/spec upgrade-template` (adopt into existing projects, no full rerun), 2.12.0 template-version drift visibility (banner + session↔installed drift + artifact stamps), 3.0.0 evaluator-backed journey discovery + Phase 1.3 loop-until-dry emergent-journey discovery. |
