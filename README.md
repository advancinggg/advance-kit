...
|---|---|---|
| `dev` | `3.2.0` | Stable — enforced /dev + /spec + /prd with dual-model evaluators, AC-driven MODULE progress, ADR system, worktree-parallel /dev, read-only `/dev board` snapshot dashboard, system-acceptance witness layer, template-version drift visibility. Includes `dev`, `spec`, `prd` skills + opt-in statusline. **Latest:** 3.2.0 /dev gains a system-acceptance harness contract — detect/declare how to bring up + run the REAL wired system for SYS-AC e2e tests (witness-floor: no mocks), plus an explicit user-accepted deferral so an `e2e` REQ is never silently stuck `Partial` when the wired system can't run here. **Earlier:** 2.12.0 template-version drift visibility (banner + session↔installed drift + artifact stamps), 3.0.0 evaluator-backed journey discovery + Phase 1.3 emergent-journey discovery, 3.1.0 atomic, independently-adjudicable SYS-AC criteria (§1.1). |
