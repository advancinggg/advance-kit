...
|---|---|---|
| `dev` | `3.11.0` | Stable — enforced /dev + /spec + /prd with dual-evaluator review (claude+codex or grok-dual backend), AC-driven MODULE progress, ADR system, worktree-parallel /dev, read-only `/dev board` snapshot dashboard, system-acceptance witness layer, template-version drift visibility. Includes `dev`, `spec`, `prd` skills + opt-in statusline. **Latest:** 3.11.0 extends the Grok Build review backend to /spec and /prd — the architecture / module / UT.10 journey-discovery / PRD-coverage evaluator loops run as two parallel native `spawn_subagent` evaluators with the same frozen positional Evaluator-A/B counters (additive progress.json `review_backend`); no Codex CLI required anywhere under Grok Build. **Earlier:** 3.10.0 Grok Build review backend for /dev (runtime auto-detection, state v7, every review point as two parallel spawn_subagent evaluators), 3.9.0 PreToolUse enforcement repair (`hookSpecificOutput` decision shape) + loop hardening, 3.8.0 mechanical fail-open DOCS-exit ledger-parity gate. |
