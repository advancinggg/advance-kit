...
|---|---|---|
| `dev` | `3.7.0` | Stable — enforced /dev + /spec + /prd with dual-model evaluators, AC-driven MODULE progress, ADR system, worktree-parallel /dev, read-only `/dev board` snapshot dashboard, system-acceptance witness layer, template-version drift visibility. Includes `dev`, `spec`, `prd` skills + opt-in statusline. **Latest:** 3.7.0 §1.5↔§3.4 ledger parity — §1.5 is the authoritative AC source and /dev DOCS births §3.4 rows in the same commit, so SUMMARY, the §6.1.1 progress gate, and the /spec module evaluator all fail closed on a row-pending AC (closing the over-claim class where a declared-but-unrowed AC let a REQ read Verified), backed by a new Ledger Parity DoD gate. **Earlier:** 3.6.0 `/dev board` surfaces the system-acceptance axis (deferrals + atomic Type breakdown + per-worktree DEFER column) with durable §3 persistence, 3.5.0 /prd marker on every §3 flow + legacy backfill, 3.4.0 Iron-Rule honest-disclosure carve-out + structured "Scope & unverified" field. |
