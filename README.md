...
|---|---|---|
| `dev` | `3.0.0` | Stable — enforced /dev + /spec + /prd with dual-model evaluators, AC-driven MODULE progress, ADR system, worktree-parallel /dev, read-only `/dev board` snapshot dashboard, system-acceptance witness layer, template-version drift visibility. Includes `dev`, `spec`, `prd` skills + opt-in statusline. **Latest:** 3.0.0 `/spec upgrade-template` runs evaluator-backed journey discovery (dual-evaluator, loop-until-dry — finds under-classified REQs + emergent cross-module journeys; degrades to single/heuristic) and Phase 1.3 hardens to loop-until-dry emergent-journey discovery (MAJOR: breaks the prior UT.10 no-evaluator contract). **Earlier:** 2.10.0 system-acceptance layer, 2.11.0 `/spec upgrade-template` (adopt into existing projects, no full rerun), 2.12.0 template-version drift visibility (banner + session↔installed drift + artifact stamps). |
