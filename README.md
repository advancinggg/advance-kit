...
|---|---|---|
| `dev` | `3.6.1` | Stable — enforced /dev + /spec + /prd with dual-model evaluators, AC-driven MODULE progress, ADR system, worktree-parallel /dev, read-only `/dev board` snapshot dashboard, system-acceptance witness layer, template-version drift visibility. Includes `dev`, `spec`, `prd` skills + opt-in statusline. **Latest:** 3.6.0 `/dev board` surfaces the system-acceptance evaluation axis — deferrals (with reason), an atomic functional/nfr/error-path Type breakdown, and a per-worktree DEFER column — backed by durable §3 deferral persistence in SYSTEM-ACCEPTANCE.md. **Earlier:** 3.5.0 /prd marker on every §3 flow + legacy backfill, 3.4.0 Iron-Rule honest-disclosure carve-out + structured "Scope & unverified" field, 3.3.0 `/spec upgrade-template` renumber-preserve on a mid-template section insert. |
