...
|---|---|---|
| `dev` | `2.8.0` | Stable — AC-based progress formula; /dev ↔ /spec section numbering aligned. Ships **`/prd` skill** for iterative PRD generation (guided dialogue, one-question-per-turn, 4-dimension coverage evaluator — adapted from Jesse Obra's brainstorming skill). MODULE template gains §1.1 "Serves PRD topics" reverse mapping, §2.13 Operations (runbook), §2.14 Observability (log/metric/trace schema). **2.4.0** adds `docs/CONTEXT-MAP.md` routing (auto-generated by /spec Phase 3.3) and `docs/GLOSSARY.md` domain-term resolution (bootstrapped by /prd Phase 3.3, appended to by /spec Phase 2.6); /dev PLAN narrows context to the matched scope entry with cross-platform python3 staleness check. **2.5.0** ships an ADR (Architecture Decision Records) workflow: `/spec adr-new` creates date-named decision files under `docs/adr/` from an inline template, auto-maintains `_INDEX.md`, Phase 1 detects semantic conflicts pairwise over Accepted ADRs (22 opposing-keyword pairs + decision-marker proximity), and wires ADR compliance into /dev PLAN + DOCS. **2.6.0** tightens /prd scope discipline: Phase 4 Dimension 4 detects architecture leakage (module IDs, DDL, API route tables, trait signatures, cross-module diagrams, directory layouts → Critical), adds optional template sub-sections §1.1 Design principles (cross-cutting product invariants) and §7.1 Milestones / rollout (user-capability roadmap, coarser than /spec IMPLEMENTATION_ORDER.md), and a Phase 3.0 boundary block separating PRD-scope from /spec-scope content. **2.7.0** adds upstream-alignment check-points (/dev DOCS §2.1.2 PRD/Spec abort+restart, §2.1.3 Core Logic drift check; /spec Phase 0.6 PRD-gap escalation — formalizes the /spec Iron Rule upstream-rollback path) so PRD/spec drift surfaced mid-workflow flows back upstream without silent documentation misalignment. **2.8.0** adds worktree-parallel /dev support: 4 subcommands (`worktree-new` / `worktree-list` / `worktree-finish` / `worktree-remove`) backed by `plugins/dev/bin/worktree-helper.sh`. Each worktree naturally gets its own `.dev-state/` via the existing `git rev-parse --show-toplevel` lookup. 2.7.0 §2.1.2 Option A (4 commands) and Option B (3 commands) sequences UNCHANGED — worktree-mode bridging (cd + git commit + git rebase using LOCAL ref via shared `.git/` object store) lives in §8.2 narrative, not as alternative commands. /spec + /prd stay single-flight. Includes `dev`, `spec`, `prd` skills plus opt-in statusline. |
