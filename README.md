...
|---|---|---|
| `dev` | `2.12.0` | Stable — enforced /dev + /spec + /prd with dual-model evaluators, AC-driven MODULE progress, ADR system, worktree-parallel /dev, read-only `/dev board` snapshot dashboard, system-acceptance witness layer, template-version drift visibility. Includes `dev`, `spec`, `prd` skills + opt-in statusline. **Latest:** 2.12.0 every skill prints the running dev-template version and warns on session↔installed drift; /spec + /prd stamp generated docs with `dev-template: vX.Y.Z` and flag artifact drift on rerun. **Earlier:** 2.9.0 `/dev board` snapshot dashboard, 2.10.0 system-acceptance layer, 2.11.0 `/spec upgrade-template` (adopt system-acceptance into existing projects, no full rerun). |
