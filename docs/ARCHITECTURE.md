...
|-------------|--------|----------------|-------------------|-------------|
| CONTRACT-001 | Y | MODULE-001 | MODULE-002, MODULE-003, MODULE-004, MODULE-005, MODULE-006, MODULE-007, MODULE-008 | StateDir resolution: returns paths for `daemon.lock`, `daemon.sock`, `admin.json`, `offset.json`, log dir; idempotent creation; enforces 0700 directory + 0600 file perms |
...
| CONTRACT-004 | Y | MODULE-002 | MODULE-004, MODULE-005, MODULE-008 | TelegramAPIClient: wraps `sendMessage`, `editMessageText`, `answerCallbackQuery`, `getFile`, `sendChatAction`, `setMessageReaction`; handles auth + rate-limit classification + 429 retry-after honoring. M005 consumes for no-session reply, `/list`/`/status`/`/session` ack via sendMessage, stale-button answerCallbackQuery |
...
| CONTRACT-010 | Y | MODULE-006 | MODULE-005 | RegistrationGate: state-machine for registration window. M005 queries `isInRegistrationWindow()` and `processRegistrationDM(sender_user_id, text): RegistrationResult` before normal routing |
| CONTRACT-011 | Y | MODULE-004 | MODULE-005 | PendingApprovalRegistry: in-memory map pending_id → {requester_session, callback_data, message_id, options}; capacity-bounded (50). M005 calls `lookupByPendingId(callback_data)`, `resolveApproval(pending_id, choice)`, and `cleanupBySession(session_id)` (invoked when M005's `session_disconnected` subscriber fires — orphans the pending entries for that session, edits the TG button message to "approval cancelled (session ended)" via M002's `editMessageText` per PRD §3.3 edge case) |
