...
|--------|--------|--------|---------|-------------|------|-----------|--------|---------|
| REQ-001 | Y | PRD.md | §3.1 | Bidirectional chat: TG→focus claude session over LRU snapshot routing, admin-only inbound (flag-spelling: `--channels plugin:telegram-channels-pro@advance-kit`) | Feature | MODULE-005 (primary), MODULE-002, MODULE-003, MODULE-004 | Verified | 2026-05-15 |
| REQ-002 | Y | PRD.md | §3.2 | Task completion push: outbound `reply` from any registered session to admin chat | Feature | MODULE-004 (primary), MODULE-002 | Verified | 2026-05-15 |
...
| REQ-008 | Y | PRD.md | §4.5 | MCP tool compatibility: reply / react / edit_message / download_attachment external behavior matches upstream 0.0.6 (JSON schema validation in M1) | Feature | MODULE-004 (primary), MODULE-002 | Verified | 2026-05-15 |
| REQ-009 | Y | PRD.md | §4.5 | `request_approval` stateful tool: inline-button + await + capacity-exceeded error at >50 pending | Feature | MODULE-004 (primary), MODULE-005 (callback routing) | Verified | 2026-05-15 |
...
| REQ-017 | Y | PRD.md | §5 | Stability SLO: 72h soak, ≥99% of 864 5-min windows zero **spurious** MCP reconnect events (scripted disconnects via `/reload-plugins` / SIGTERM / launchd KeepAlive restart excluded); single continuous >5min outage = fail. Handshake mechanism (REQ-045) classifies events | NFR | MODULE-002 (primary, polling role), MODULE-001, MODULE-003, MODULE-008 | Draft | 2026-05-15 |
...
| REQ-020 | Y | PRD.md | §5 | Latency: inbound TG→claude P95<5s, reply→TG P95<2s (delivered-only), approval P95<3s (60s-click-window-only). Slice 2 partial: M005 in-process micro-benchmark (max<5ms) only — full E2E latency NFR deferred to soak. | NFR | MODULE-002 (primary, polling), MODULE-003 (transport), MODULE-005 (routing) | Partial | 2026-05-15 |
...
| REQ-023 | Y | PRD.md | §5 | Structured JSON logs: event_type / session_id / request_id / error_class fields; redaction list (token, user IDs, DM body, identity path, reg code); `status` subcommand summary. Redaction scope = JSON event log only (see REQ-044 two-stream invariant) | NFR | MODULE-008 (primary), MODULE-006 (registration code emission paths under REQ-044 cross-link) | Verified | 2026-05-14 |
...
| REQ-037 | Y | PRD.md | §3.2 | Quarantine outbound replay queue: in-memory 50-cap; new reply beyond cap returns `CapacityExceededError`; daemon restart drops queue (best-effort; claude end discovers via next call error). Drain notification mechanism (REQ-045) notifies requester session on quarantine end | Feature | MODULE-002, MODULE-001, MODULE-003 | Verified | 2026-06-01 |
...
## Module(s) Column Convention

Module(s) lists ALL modules that participate in a REQ's fulfillment, with **primary owner first by position**. An optional `(primary)` token may be appended to the first module for explicit emphasis (style-only, not required — used inconsistently across rows for historical reasons). Auxiliary modules MAY carry a role-tag in parentheses (e.g., `MODULE-002 (TG sendMessage delivery)`). Single-module rows are trivially unambiguous and need no annotation. **Sync rule**: any update to a REQ's Module(s) list (primary, ordering, or auxiliary set) must be mirrored in ARCHITECTURE.md §10 in the same commit; the /spec workflow's evaluator loops gate on this consistency.

**Primary owner for the canonical multi-module rows** (set by ARCHITECTURE.md §10 where listed; REGISTRY ordering matches; 30 entries):
- REQ-001 = M005; REQ-002 = M004; REQ-003 = M004; REQ-006 = M001; REQ-008 = M004; REQ-009 = M004; REQ-011 = M006; REQ-013 = M006; REQ-016 = M001; REQ-017 = M002; REQ-020 = M002; REQ-021 = M001; REQ-022 = M005; REQ-023 = M008; REQ-024 = M008; REQ-025 = M001; REQ-032 = M003; REQ-033 = M003; REQ-034 = M005; REQ-035 = M002; REQ-036 = M004; REQ-037 = M002; REQ-038 = M004; REQ-039 = M004; REQ-041 = M003; REQ-043 = M005; REQ-044 = M006; REQ-045 = M008; REQ-046 = M006; REQ-047 = M006.
