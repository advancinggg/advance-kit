...
| `shortid_allocation_failed` (REQ-041) | `assignUniqueShortid()` throws after 1024 consecutive `randomBytes(6)` collisions (astronomically unlikely — suspect RNG corruption or a misbehaving test mock) | `acceptSession` wraps the call in try/catch: emits `log_emit` `{level:'ERROR', event_type:'shortid_allocation_failed'}`, performs best-effort `sock.end()` + `sock.destroy()` (each individually try-wrapped), then early-returns WITHOUT writing `session_init_ack` or emitting `session_connected`. No `sessionMap` entry or timer is created on this path (the throw fires before any state mutation), so nothing leaks. Existing sessions are unaffected; the daemon does NOT crash (the prior behavior — an uncaught throw escaping the `sock.on("data")` callback as an uncaughtException — was fixed in audit-fix Round 1). |
