...
| 2026-05-21 | /dev task `dev-advance-kit-20260521-0edfd84f` — REQ-047 stream (c) impl. Public method `isWaitForReset(): boolean` added to RegistrationGate class implementation (interface declaration at §2.3 line 293 was already present per v1.1.0 /spec — code catches up). Trivial body: `return this.currentState === "waiting_for_reset";`. M005 WaitForResetHandshakeHandler (new) consumes this query to drive stream (c) of REQ-047 multi-stream delivery (MCP handshake disconnect with `"registration timed out; run reset-admin to retry"` reason). REQ-047 streams (a) stderr periodic + (b) macOS Notification Center one-shot remain M006-internal per Decision A21 — OUT OF SCOPE for this slice (M006 §3.4 drift rows AC-21..28 not addressed; /spec rerun required to align §1.5↔§3.4). REQ-047 Registry Status expected to remain Partial post-SUMMARY because streams (a)+(b) coverage is unverified. |
