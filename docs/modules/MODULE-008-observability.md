...
| 2026-05-16 | /dev task — channel-protocol slice (REQ-033/037/045). **Source-only typecheck-driven update**: `src/obs/subscriber.ts` `LEVEL_BY_TYPE` Record extended with 3 entries for the new event-type catalog additions (`quarantine_replay_resolved: "INFO"`, `mcp_reconnect_classified: "INFO"`, `channel_notification_emitted: "DEBUG"`) so `Record<EventTypeKey, ...>` exhaustiveness keeps compiling. `ALERT_FEED` unchanged — the 3 new event types are observability signals, not alert triggers. No M008 AC in this slice's scope; AC-01 (subscriber subscribes to ALL canonical event types) continues to pass against the extended catalog. M008 subscribers for `mcp_reconnect_classified` SLO counter (REQ-017+REQ-045 AC-21) and full REQ-037 quarantine_replay_queue gauge are M008-side ACs not included here. |
| 2026-05-21 | /dev task `dev-advance-kit-20260521-0edfd84f` — REQ-035 catalog extension. **Source-only typecheck-driven update**: `src/daemon/event-types.ts` adds `"outbound_chat_type_denied"` to `EventTypeKey` union + payload schema `{chat_id: number; observed_type: string; tool: string}` + inserts into `ALL_EVENT_TYPES` array immediately after `chat_type_lookup` at the tail (preserving the existing logical grouping convention — NOT alphabetical). `src/obs/subscriber.ts` `LEVEL_BY_TYPE` Record adds `outbound_chat_type_denied: "WARN"` for exhaustive Record-key coverage. `ALERT_FEED` unchanged. No M008 AC in this slice's scope; AC-01 continues to pass against the extended catalog. M008 §3.4 drift rows AC-20..29 (StatusReporter v1.1.0 fields population) remain unaddressed — out of scope per Path A. |
