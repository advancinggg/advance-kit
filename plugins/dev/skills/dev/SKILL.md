...
`worktree path  task_id  phase  eval_round  updated_at`. Missing state.json →
`(no /dev workflow)` with `—` placeholders. `null` fields normalised to `—`.
...
- mutate MODULE docs or REQUIREMENTS_REGISTRY (read-only file open).
...
- Section 2 phase colours: plan/docs cyan; implement/audit/test/adversarial
  yellow; summary green; everything else uncoloured.
