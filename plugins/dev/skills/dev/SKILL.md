...
- §3.4 AC Verification ledger is the single source of truth: add new AC rows (Active=Y, Status=untested); removed AC rows become Active=N (do not delete; preserves §3.4 merge-preserve semantics from /spec). §3.1 Current Status is re-derived by SUMMARY via the formula in §6.1.1, no manual editing. §3.5 Feature Implementation Record stays hand-authored and is not regenerated by this workflow. Record this change in §3.7 Change History.
...
- Update ARCHITECTURE.md's module progress overview by taking the **arithmetic mean** of per-module §3.1 percentages, **excluding modules whose §3.1 shows `—` (zero Active=Y AC)**. Arithmetic mean makes the dashboard number reproducible; AC-weighted rollup is not written here.
