...
7. **All side-effects happen here** (after steps 3 + 4 + 5 + 6 have passed).

   **Concurrency lock acquired at step 3 entry, released on any exit path** (moved up in the flow so step 3's collision scan + step 5's filename decision are inside the critical section, preventing two concurrent `/spec adr-new` invocations from observing the same empty-slot state and both claiming the same filename). Implementation: first do `mkdir -p docs/adr/` (the parent dir must exist to host the lock), then `mkdir docs/adr/.adr-new.lock.d 2>/dev/null` (portable atomic lock — returns non-zero if directory already exists). If lock acquisition fails → print `Another /spec adr-new is currently active (lockfile docs/adr/.adr-new.lock.d); retry in a moment. If stale, remove the lockfile manually.` and exit 1. **Stale-lock detection**: before the exit-1 fail path, check the lockfile's mtime. If older than 60 seconds → warn `Stale lock detected (age >60s); auto-removing.`, `rmdir` it, retry `mkdir`. This prevents permanent self-DoS if the holding process died (SIGKILL, terminal close). Release protocol on success: `trap 'rmdir docs/adr/.adr-new.lock.d' EXIT` at the top of the side-effect section.
