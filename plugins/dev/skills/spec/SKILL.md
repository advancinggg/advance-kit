...
**Preamble — concurrency lock (acquired BEFORE step 1, released on any exit path)**: first `mkdir -p docs/adr/` (the parent must exist to host the lock), then `mkdir docs/adr/.adr-new.lock.d 2>/dev/null` (portable atomic lock — returns non-zero if the directory already exists). If lock acquisition fails, check the lockfile's mtime: if older than 60 seconds → warn `Stale lock detected (age >60s); auto-removing.`, `rmdir` it, and retry `mkdir`. If the retry also fails OR the lockfile is fresh → print `Another /spec adr-new is currently active (lockfile docs/adr/.adr-new.lock.d); retry in a moment. If stale, remove the lockfile manually.` and exit 1. On success, install `trap 'rmdir docs/adr/.adr-new.lock.d' EXIT` so any exit path (normal, error, SIGINT) releases the lock. This serializes the entire collision-scan + filename-decision + write critical section — two concurrent `/spec adr-new` invocations cannot both observe the same empty slot and claim the same filename.

**ADR path-confinement check (2.5.0+ adversarial-hardening)**: all ADR file reads/writes under `docs/adr/` must verify the resolved real path is still under `docs/adr/` (reject symlinks that escape). In bash: `abs=$(python3 -c "import os,sys; print(os.path.realpath(sys.argv[1]))" "$f"); case "$abs" in "$REPO/docs/adr/"*) : ;; *) echo "Refusing to read/write $f — symlink escape detected: resolves to $abs"; exit 1 ;; esac`. This applies to: Phase 1.0 step 2 parser (read), Phase 1.0 step 5 ADR mutations (write), Phase 1.0 step 7 `_INDEX.md` rebuild (read-all + overwrite), Phase ADR-NEW step 7 template write + `_INDEX.md` append, and `/dev` §1.1 ADR fallback scan (read). Downstream, adds negligible latency (one realpath per file) and blocks the symlink-to-arbitrary-file tampering vector.
